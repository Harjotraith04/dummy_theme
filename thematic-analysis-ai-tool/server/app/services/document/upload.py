...
class DocumentUploadService:
    ...
    def create_document(
        ...
        # Create the document record (without content - content is stored as segments)
        document = Document(
            name=name,
        ...
        if segments_data and "segments" in segments_data:
            ...
            # Refresh the document to get the created segments
            db.refresh(document)
    ...
    def _create_document_segments(db: Session, document: Document, segments_data: list):
        """Create DocumentSegment database records from extracted segment data"""
        for segment_data in segments_data:
            # Map legacy field names to database fields
            segment = DocumentSegment(
                document_id=document.id,
                # Map "type" to "segment_type"
            ...
            db.commit()
        print(
            f"Created {len(segments_data)} segments for document {document.id}")
    ...
    def _extract_text_content(file_content: bytes) -> tuple[Dict[str, Any], str]:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if line_content:  # Only include non-empty lines
                    segments.append({
                        "type": "line",  # Keep for compatibility with _create_document_segments
